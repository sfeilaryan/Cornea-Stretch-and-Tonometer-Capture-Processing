...
for video in list_of_video:
    ...
    thickness, x_min = thickness_evolution(
        video, x_left=0, x_right=576, p_left=x_left, p_right=x_right
    )
    ...
    index6_list.append(
        np.mean(
            thickness[:, int((x_min - x_left) / 10) : int((x_min - x_left) / 10) + 1]
        )
    )
...
df["index5(horizontal distance between initial thickness minimum and apex)"] = np.array(
    index5_list
)
df["index6(average thickness change around initial thickness minimum)"] = np.array(
    index6_list
)
