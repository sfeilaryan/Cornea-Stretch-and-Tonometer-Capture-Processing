...
def video_to_image(video, directory, output_directory, output_name_prefix=""):
    """
    ...
    while True:
        ...
        if ret:
            cv2.imwrite(
                os.path.join(output_dir, output_name_prefix + str(img).zfill(3)), frame
            )
            img += 1
        else:
            print(f"Video {video} converts over, {img} images are extracted in total.")
            break
