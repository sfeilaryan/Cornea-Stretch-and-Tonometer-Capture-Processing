...
def video_to_image(video):
    ...
    while True:
        ...
        if ret:
            ...
            print(f"Video {video} converts over, {img-1} images are extracted in total.")
            break
...
def displacement_evolution(video, output=True):
    ...
    for t in range(10, 61):
        ...
        for x in range(width):
            if t == 10:
                ...
                displacement_up[t-10, x] = get_apex_y(image, x) - displacement_up[0, x]
    ...
    for t in range(109, 69, -1):
        ...
        for x in range(width):
            if t == 109:
                ...
                displacement_down[t-70, x] = get_apex_y(image, x) - displacement_down[39, x]
    ...
    if output:
        ...
        x_down_grid, y_down_grid = np.meshgrid(pressure_time_curve_down(), np.arange(width))
        fig = plt.figure(dpi=200)
        fig.suptitle(f'Displacement evolution of video {video}')
        fig.text(0.03, 0.5, 'coordinate x', va='center', rotation='vertical', fontsize=12)
        fig.text(0.5, 0.03, 'pressure', va='center', ha='center', fontsize=12)
        ax = fig.add_subplot(121)
        bx = fig.add_subplot(122)
        bx.invert_xaxis()
        cm = plt.cm.get_cmap('jet')
        ax.pcolormesh(x_up_grid, y_up_grid, displacement_up.T, cmap=cm, vmin=-100, vmax=100)
        b = bx.pcolormesh(x_down_grid, y_down_grid, displacement_down.T, cmap=cm, vmin=-100, vmax=100)
    ...
    for t in range(time):
        ...
        for x in range(width):
            if waveform[x, 0] > 0 and waveform[x, 1] > 0:
                image[waveform[x, 0]:waveform[x, 1], x, :] = float2RGB(distance_variation[x])
                """
...
def waveform_evolution(video, output=False):
    ...
    if output:
        ...
        animation = ani.FuncAnimation(fig=fig, func=update, frames=np.arange(time), interval=10)
...
def thickness_evolution(video, x_left, x_right, p_left, p_right):
    ...
    for t in range(19, 101, 1):
        ...
        image = cv2.imread(image_path)
        image = image_processing(image)
        thickness[t] = np.array(get_thickness(image, x_left=x_left, x_right=x_right)) - thickness0
...
def get_viscosity(video):
    ...
    print(f'-- the viscosity of the cornea in video {video} is {viscosity/1000000:.4f} MPa.s')
...
def get_parabola_form(video, output=False):
    ...
    parabola_list = [parabola.subs(x, x_value) for x_value in np.linspace(100, 499, 400)]
