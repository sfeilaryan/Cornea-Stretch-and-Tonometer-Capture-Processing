...
def getFramePotential(videoName, imageNumber, initialLengths, K_arr, iGuess = np.zeros(N), surface = "mid"):
...
def potentialEvolution(videoName, totalImages, increment = 5, surface = "mid"):   #returns Potential Array (Chronological)
    ...
    while imageNumber <= totalImages:
        ...
        U, newGuess = getFramePotential(videoName, imageNumber, initialLengths, K_arr, iGuess, surface)
...
def getMiddleWaveformCoefs_Line(videoName, imageNumber, OUTPUT = False):   #no path - just number
    imagePath = "Data/video"+videoName+"_TREATED/"+"image"+ str(imageNumber) +".jpg"
    frame = cv2.imread(imagePath)
    frame = utils.image_processing(frame) #To get two dimensional array instead of 3 (Grayscale)
...
def getErrorFrames(videoName, totalImages = 139, increment = 1):
    ...
    while imageNumber <= totalImages:
        ...
        plt.ylim((0,45))
        plt.xlim((0,600))
        plt.savefig("Error_Profiles/"+videoName+"/FRAMES/"+str(imageNumber)+".png")
        plt.close()
...
def formattedImageCreator_ERROR(videoName):
    ...
    while imageIterator <= num_of_images:
        ...
        # resizing  
        imResize = im.resize((mean_width, mean_height), Image.ANTIALIAS)  
        imResize.save( "Error_Profiles/"+videoName+"/FRAMES_RESIZED/"+str(imageIterator)+".png", quality = 95) # setting quality
        imageIterator+=1
...
def getUpperWaveformFrames(videoName, totalImages=139):
    ...
    if os.path.exists("Upper_Waveforms/FRAMES/"+videoName+"/"):
        ...
        imageNumber = 1
        while imageNumber <= totalImages:
            middle = getUpperWaveform(videoName, imageNumber, OUTPUT = False)    #it is a thruple.
            plt.figure(figsize = (12,8))
            plt.plot(middle[0], middle[1])
            plt.title("Waveform on frame " + str(imageNumber))
            plt.ylim((0,200))
            plt.xlim((0,600))
            plt.savefig("Upper_Waveforms/FRAMES/"+videoName+"/"+str(imageNumber)+".png")
            plt.close()
            imageNumber+=1
...
def formattedImageCreator_upper(videoName):
    ...
    while imageIterator <= num_of_images:
        ...
        # resizing  
        imResize = im.resize((mean_width, mean_height), Image.ANTIALIAS)  
        imResize.save( "Upper_Waveforms/FRAMES_RESIZED/"+videoName+"/"+str(imageIterator)+".png", quality = 95) # setting quality
        imageIterator+=1
...
def plotUpperFramePotential(videoName, imageNumber, initialLengths, K_arr, output = False, iGuess = np.zeros(N)):
...
#     plt.show()
    ...
    plt.ylim((0,0.1))
    plt.xlim((0,600))
    plt.savefig("Upper_Potential_Profiles/"+videoName+"/V_x/FRAMES/"+str(imageNumber)+".png")
    plt.close()
    
    return displacements.x #for the next guess
    
    
def get_upper_V_x_Frames(videoName, totalImages = 139, increment = 1):   #NO RETURN, JUST GET FIGURES
    ...
    while imageNumber <= totalImages:
        ...
        print(f'Processing Frame: {imageNumber}')
        newGuess = plotUpperFramePotential(videoName, imageNumber, initialLengths, K_arr, False, iGuess)
        imageNumber += increment
        iGuess = newGuess
...
def formattedImageCreator_V_x_UPPER(videoName):
    ...
    while imageIterator <= num_of_images:
        ...
        # resizing  
        imResize = im.resize((mean_width, mean_height), Image.ANTIALIAS)  
        imResize.save( "Upper_Potential_Profiles/"+videoName+"/V_x/FRAMES_RESIZED/"+str(imageIterator)+".png", quality = 95) # setting quality
        imageIterator+=1
...
def generate_video_V_x_UPPER(videoName):
    ...
    if os.path.exists("Upper_Potential_Profiles/"+videoName+"/V_x/"+"V_x_evolution.mp4"):
        print("Already processed.")
        return
...
def getUpperWaveformCoefs_Line(videoName, imageNumber, OUTPUT = False):   #no path - just number
    imagePath = "Data/video"+videoName+"_TREATED/"+"image"+ str(imageNumber) +".jpg"
    frame = cv2.imread(imagePath)
    frame = utils.image_processing(frame) #To get two dimensional array instead of 3 (Grayscale)
...
def getUpperErrorFrames(videoName, totalImages = 139, increment = 1):
    ...
    while imageNumber <= totalImages:
        ...
        plt.ylim((0,45))
        plt.xlim((0,600))
        plt.savefig("Upper_Error_Profiles/"+videoName+"/FRAMES/"+str(imageNumber)+".png")
        plt.close()
...
def formattedImageCreator_upperERROR(videoName):
    ...
    while imageIterator <= num_of_images:
        ...
        # resizing  
        imResize = im.resize((mean_width, mean_height), Image.ANTIALIAS)  
        imResize.save( "Upper_Error_Profiles/"+videoName+"/FRAMES_RESIZED/"+str(imageIterator)+".png", quality = 95) # setting quality
        imageIterator+=1
...
def getEvolution_V_t(videoName, increment, surface):
    ...
    if surface == "mid":
        ...
        plt.savefig("Potential_Profiles/"+videoName+"/V_t/TimeEvolution_"+surface+".png")
        ...
        plt.savefig("Upper_Potential_Profiles/"+videoName+"/V_t/TimeEvolution_"+surface+".png")
